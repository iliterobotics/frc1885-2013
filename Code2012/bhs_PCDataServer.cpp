...
bool bhs_PCDataServer::xmitData(char* apData, int anBufferSize)
{
	bool lbStatus = false;
	if (mbServerReady)
	{
		semTake(mExternalUserSem,WAIT_FOREVER);//thread safe for external users and pcDataServer task
		xmitRawData(0xCAFEFACE, anBufferSize, apData);
...
bool bhs_PCDataServer::xmitImage(char* apImageData, int anImageSizeInBytes)
{
	bool lbStatus = false;
	if(mbServerReady)
	{
		semTake(mExternalUserSem,WAIT_FOREVER);//thread safe for external users and pcDataServer task
		xmitRawData(0xDEADBEEF, anImageSizeInBytes, apImageData);
		semGive(mExternalUserSem);
...
bool bhs_PCDataServer::xmitImage2(char* apImageData, int anImageSizeInBytes)
{
	bool lbStatus = false;
	if(mbServerReady)
	{
		semTake(mExternalUserSem, WAIT_FOREVER);//thread safe for external users and pcDataServer task
		xmitRawData(0xFADFEADD, anImageSizeInBytes, apImageData);
		semGive(mExternalUserSem);

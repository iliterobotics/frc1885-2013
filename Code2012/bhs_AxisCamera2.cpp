...
const int bhs_AxisCamera2::FRAMES_PER_SECOND = 10;
bhs_AxisCamera2 *bhs_AxisCamera2::_instance = NULL;

/**
 * bhs_AxisCamera2 constructor
 */
bhs_AxisCamera2::bhs_AxisCamera2(const char *ipAddress)
	: bhs_AxisCameraParams2(ipAddress)
 ...
 * Destructor
 */
bhs_AxisCamera2::~bhs_AxisCamera2()
{
	m_dataServer = NULL;
...
/**
 * Get a pointer to the bhs_AxisCamera2 object, if the object does not exist, create it
 * To use the camera on port 2 of a cRIO-FRC, pass "192.168.0.90" to the first GetInstance call.
 * @return reference to bhs_AxisCamera2 object
 */
bhs_AxisCamera2 *bhs_AxisCamera2::GetInstance(const char *cameraIP)
{
	if (NULL == _instance)
	{
		_instance = new bhs_AxisCamera2(cameraIP);
	}
 ...
 */
void bhs_AxisCamera2::DeleteInstance()
{
	delete _instance;
	_instance = NULL;
}

void bhs_AxisCamera2::setDataServer(bhs_PCDataServer* a_dataServer) {
	m_dataServer = a_dataServer;
}
 ...
 */
bool bhs_AxisCamera2::IsFreshImage()
{
	return m_freshImage;
}
 ...
 *
 * The semaphore is owned by the bhs_AxisCamera2 class and will be deleted when the class is destroyed.
 * @return A semaphore to notify when new image is received
 */
SEM_ID bhs_AxisCamera2::GetNewImageSem()
{
 ...
 */
int bhs_AxisCamera2::GetImage(Image* imaqImage)
{
 ...
 * @return 1 upon success, zero on a failure
 */
int bhs_AxisCamera2::GetImage(ColorImage* image)
{
 ...
 */
HSLImage* bhs_AxisCamera2::GetImage()
{
 ...
 */
int bhs_AxisCamera2::CopyJPEG(char **destImage, int &destImageSize, int &destImageBufferSize)
{
	Synchronized sync(m_protectedImageSem);
	if (destImage == NULL)
 ...
 */
int bhs_AxisCamera2::s_ImageStreamTaskFunction(bhs_AxisCamera2 *thisPtr)
{
	return thisPtr->ImageStreamTaskFunction();
}

/**
 * Task spawned by bhs_AxisCamera2 constructor to receive images from cam
 ...
 */
int bhs_AxisCamera2::ImageStreamTaskFunction()
{
...
Authorization: Basic RlJDOkZSQw==\n\n";
		...
		taskDelay(sysClkRateGet() / FRAMES_PER_SECOND);
	}
}
 ...
 */
int bhs_AxisCamera2::ReadImagesFromCamera()
{
	char *imgBuffer = NULL;
	...
	while (1)
	{
		...
		if (m_dataServer != NULL) {
			m_dataServer->xmitImage2(imgBuffer, readLength);
		}
#endif
 ...
 */
void bhs_AxisCamera2::UpdatePublicImageFromCamera(char *imgBuffer, int imgSize)
{
	{
		Synchronized sync(m_protectedImageSem);
 ...
 */
void bhs_AxisCamera2::RestartCameraTask()
{
	m_imageStreamTask.Stop();
	m_imageStreamTask.Start((int)this);
}
...
#endif
	bhs_AxisCamera2::GetInstance(IPAddress);
}

int AxisCameraGetImage (Image* image)
{
	return bhs_AxisCamera2::GetInstance().GetImage(image);
}

void AxisCameraWriteBrightness(int brightness)
{
	bhs_AxisCamera2::GetInstance().WriteBrightness(brightness);
}

int AxisCameraGetBrightness()
{
	return bhs_AxisCamera2::GetInstance().GetBrightness();
}

void AxisCameraWriteWhiteBalance(bhs_AxisCameraParams2::WhiteBalance_t whiteBalance)
{
	bhs_AxisCamera2::GetInstance().WriteWhiteBalance(whiteBalance);
}

bhs_AxisCameraParams2::WhiteBalance_t AxisCameraGetWhiteBalance()
{
	return bhs_AxisCamera2::GetInstance().GetWhiteBalance();
}

void AxisCameraWriteColorLevel(int colorLevel)
{
	bhs_AxisCamera2::GetInstance().WriteColorLevel(colorLevel);
}

int AxisCameraGetColorLevel()
{
	return bhs_AxisCamera2::GetInstance().GetColorLevel();
}

void AxisCameraWriteExposureControl(bhs_AxisCameraParams2::Exposure_t exposure)
{
	bhs_AxisCamera2::GetInstance().WriteExposureControl(exposure);
}

bhs_AxisCameraParams2::Exposure_t AxisCameraGetExposureControl()
{
	return bhs_AxisCamera2::GetInstance().GetExposureControl();
}

void AxisCameraWriteExposurePriority(int exposure)
{
	bhs_AxisCamera2::GetInstance().WriteExposurePriority(exposure);
}

int AxisCameraGetExposurePriority()
{
	return bhs_AxisCamera2::GetInstance().GetExposurePriority();
}

void AxisCameraWriteMaxFPS(int maxFPS)
{
	bhs_AxisCamera2::GetInstance().WriteMaxFPS(maxFPS);
}

int AxisCameraGetMaxFPS()
{
	return bhs_AxisCamera2::GetInstance().GetMaxFPS();
}

void AxisCameraWriteResolution(bhs_AxisCameraParams2::Resolution_t resolution)
{
	bhs_AxisCamera2::GetInstance().WriteResolution(resolution);
}

bhs_AxisCameraParams2::Resolution_t AxisCameraGetResolution()
{
	return bhs_AxisCamera2::GetInstance().GetResolution();
}

void AxisCameraWriteCompression(int compression)
{
	bhs_AxisCamera2::GetInstance().WriteCompression(compression);
}

int AxisCameraGetCompression()
{
	return bhs_AxisCamera2::GetInstance().GetCompression();
}

void AxisCameraWriteRotation(bhs_AxisCameraParams2::Rotation_t rotation)
{
	bhs_AxisCamera2::GetInstance().WriteRotation(rotation);
}

bhs_AxisCameraParams2::Rotation_t AxisCameraGetRotation()
{
	return bhs_AxisCamera2::GetInstance().GetRotation();
}

void AxisCameraDeleteInstance()
{
	bhs_AxisCamera2::DeleteInstance();
}

int AxisCameraFreshImage()
{
	return bhs_AxisCamera2::GetInstance().IsFreshImage();
}

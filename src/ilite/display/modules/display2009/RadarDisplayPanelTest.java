...
public class RadarDisplayPanelTest extends JPanel implements Runnable
{
  //Constants
  private static final int CENTER_POINT_X = 423;
  private static final int CENTER_POINT_Y = 483;
  ...
  int mnCameraZone1Active=1;
  int mnCameraZone2Active=1;
  int mnCameraZone3Active=1;
  ...
  public RadarDisplayPanelTest()
  {
    super();
    
    Dimension size = new Dimension(847,583);
  ...
  public void paint (Graphics g)
  {
    ...
    RANGE_OF_RADAR = ControlPanel.snRadarRange*12;
    RANGE_OF_TURRET = ControlPanel.snTurretRange*12;
  ...
  public void paintRadar(Graphics g)
  {
    ...
    g2d.setColor(activeZonesColor[mnCameraZone1Active]);
    g2d.fillArc(CENTER_POINT_X-WIDTH-WIDTH_OF_ZONE_INDICATOR,
      CENTER_POINT_Y-WIDTH-WIDTH_OF_ZONE_INDICATOR,2*(WIDTH+WIDTH_OF_ZONE_INDICATOR),
      2*(WIDTH+WIDTH_OF_ZONE_INDICATOR),120,60);
    
    g2d.setColor(activeZonesColor[mnCameraZone2Active]);
    g2d.fillArc(CENTER_POINT_X-WIDTH-WIDTH_OF_ZONE_INDICATOR,
      CENTER_POINT_Y-WIDTH-WIDTH_OF_ZONE_INDICATOR,2*(WIDTH+WIDTH_OF_ZONE_INDICATOR),
      2*(WIDTH+WIDTH_OF_ZONE_INDICATOR),60,59);
    
    g2d.setColor(activeZonesColor[mnCameraZone3Active]);
    g2d.fillArc(CENTER_POINT_X-WIDTH-WIDTH_OF_ZONE_INDICATOR,
      CENTER_POINT_Y-WIDTH-WIDTH_OF_ZONE_INDICATOR,2*(WIDTH+WIDTH_OF_ZONE_INDICATOR),
    ...
    g2d.fillArc(CENTER_POINT_X-WIDTH,CENTER_POINT_Y-WIDTH,2*(WIDTH),2*(WIDTH),0,180);
    ...
    //Paints arcs  
    for(int c=NUM_OF_ARCS-1;c>=0;c--)
    {  
      widthOfArc=(c+1)/(double)NUM_OF_ARCS;
      arcDistance = RANGE_OF_RADAR/NUM_OF_ARCS;
      
      g2d.setColor(new Color((int)(150-(150.0/(NUM_OF_ARCS+2)*c)),
        150-(int)(150.0/NUM_OF_ARCS*c),150-(int)(150.0/NUM_OF_ARCS*c)));
        
      g2d.fillArc((int)(CENTER_POINT_X-widthOfArc*WIDTH),
        (int)(CENTER_POINT_Y-widthOfArc*WIDTH),(int)(widthOfArc*2*WIDTH),
        (int)(widthOfArc*2*WIDTH),MIN_ANGLE,MAX_ANGLE-MIN_ANGLE);
      
      g2d.setColor(Color.GREEN);
      g2d.drawArc((int)(CENTER_POINT_X-widthOfArc*WIDTH),
        (int)(CENTER_POINT_Y-widthOfArc*WIDTH),(int)(widthOfArc*2*WIDTH),
        (int)(widthOfArc*2*WIDTH),0,180);
        
      g2d.setColor(Color.WHITE);
      g2d.drawString(""+(arcDistance*(c+1)/12), 
        (int)(CENTER_POINT_X+WIDTH*widthOfArc)-10, (int)(CENTER_POINT_Y+15));
      g2d.drawString(""+(arcDistance*(c+1)/12), 
        (int)(CENTER_POINT_X-WIDTH*widthOfArc)-10, (int)(CENTER_POINT_Y+15));
    }
    ...
    g2d.setColor(Color.GREEN);
    //Paints lines
    g2d.drawLine(CENTER_POINT_X-WIDTH-WIDTH_OF_ZONE_INDICATOR,CENTER_POINT_Y,
      CENTER_POINT_X+WIDTH+WIDTH_OF_ZONE_INDICATOR,CENTER_POINT_Y);
    for(int c=1;c<8;c++)
    {
      angle = c/8.0*Math.PI;
      g2d.drawLine(CENTER_POINT_X,CENTER_POINT_Y,
      ...
      for(int k = 0; k < 15; k++) 
      {
        ...
        if(maValidity[k]==1)
        {
          ...
          if(maAngle[k]>=45&&maAngle[k]<=135&&maDistance[k]<=RANGE_OF_TURRET)
          {
            ...
            g.setColor(Color.WHITE);
            g.drawString("Distance: "+maDistance[k],(int)(x-SIZE_OF_TARGET/2), (int) (y+SIZE_OF_TARGET/2+15));  
            g.drawString("Angle:"+(maAngle[k]-90),(int)(x-SIZE_OF_TARGET/2), (int) (y+SIZE_OF_TARGET/2+25));
          } 
          //If it's outside of the range, draw a "ghost" target, with just an outline. :D
          else if(maAngle[k]>=0&&maAngle[k]<=180 && maDistance[k]<=RANGE_OF_RADAR)
          {
            g2d.setColor(g.getColor());
            //Target is a friend. Don't kill them.
            if(maRedOrBlue[k]==mnTeamColor)
              g2d.drawRect((int)(x-SIZE_OF_FRIEND/2), (int)(y-SIZE_OF_FRIEND/2), (int)SIZE_OF_FRIEND,(int)SIZE_OF_FRIEND);
            //Target is either an enemy, or we have no idea. To the death!
            else
              g2d.drawRect((int)(x-SIZE_OF_TARGET/2), (int)(y-SIZE_OF_TARGET/2), (int)SIZE_OF_TARGET,(int)SIZE_OF_TARGET);
...
//    mnCurrentTargetDistance = acData.mnCurrentTargetDistance;
  ...
  public static void main(String[] args)
  {
    ...
    RadarDisplayPanelTest radar = new RadarDisplayPanelTest();
